    ...
    >
      <div style={{ 
        ...
        <div style={{ flex: 1 }}>
            <span style={{ 
                ...
                {data?.name?.split(' ').map(n => n[0]).join('. ') || 'C. SAKINALA'}.
            </span>
            <p style={{ 
                opacity: 0.6, 
                fontSize: '0.9rem', 
            ...
            <div style={{ display: 'flex', gap: '1.2rem' }}>
                {[
                    { icon: <Github size={18} />, url: data?.links?.github || "https://github.com/saithevfrvrygngreat", label: "GitHub" },
                    { icon: <Linkedin size={18} />, url: data?.links?.linkedin || "https://www.linkedin.com/in/dasetti-sai-kumar-b30340299/", label: "LinkedIn" }
                ].map((social, idx) => (
                    <motion.a
                        key={idx}
                        href={social.url}
        ...
        <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <a href={`mailto:${data?.email || 'dasettisaikumar@gmail.com'}`} style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', color: theme.text, opacity: 0.6, textDecoration: 'none', fontSize: '0.9rem' }}>
                    <Mail size={15} color={theme.accent} /> 
                    <span>{data?.email || 'dasettisaikumar@gmail.com'}</span>
                </a>
                <a href={`tel:${data?.mobile || '+919059245025'}`} style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', color: theme.text, opacity: 0.6, textDecoration: 'none', fontSize: '0.9rem' }}>
                    <Phone size={15} color={theme.accent} />
                    <span>{data?.mobile || '+91 9059245025'}</span>
                </a>
            </div>
        </div>
      ...
      <div style={{ 
        ...
        <p style={{ opacity: 0.4, fontSize: '0.8rem', letterSpacing: '0.05em' }}>
            © {year} Developed by <span style={{ fontWeight: 700, color: theme.text }}>Chanti Sakinala</span>. 
        </p>
      </div>
    </footer>
  );
